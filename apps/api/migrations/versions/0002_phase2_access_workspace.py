...
def upgrade():
    ...
    if not exists("access_grants"):
        op.create_table(
            ...
            sa.Column(
                "permission_id", sa.String(36), sa.ForeignKey("permissions.id", ondelete="CASCADE"), nullable=False
            ),
            sa.Column("scope", sa.String(20), nullable=False),
            sa.Column("effect", sa.String(10), nullable=False),
            sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
            sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False),
            sa.UniqueConstraint("organization_id", "principal_type", "principal_id", "permission_id"),
        )
    ...
    if dialect != "sqlite":
        for name, table, local, remote in [
            ("fk_teams_manager", "teams", "manager_id", "users.id"),
            ("fk_tasks_department", "tasks", "department_id", "departments.id"),
            ("fk_events_creator", "events", "creator_id", "users.id"),
            ("fk_events_owner", "events", "owner_user_id", "users.id"),
            ("fk_events_team", "events", "team_id", "teams.id"),
            ("fk_events_department", "events", "department_id", "departments.id"),
            ("fk_announcements_team", "announcements", "team_id", "teams.id"),
            ("fk_announcements_department", "announcements", "department_id", "departments.id"),
            ("fk_announcements_creator", "announcements", "creator_id", "users.id"),
        ]:
...
def downgrade():
    ...
    for table, names in {
        ...
        "tasks": ["visibility", "department_id"],
        "navigation_items": ["title_he", "title_en"],
        "teams": ["is_active", "manager_id", "description"],
        "users": ["locale"],
