...
PERMISSIONS = [
    ...
    "dashboards.publish",
    "dashboards.assign",
    "dashboards.export_pdf",
    "dashboards.export_excel",
...
async def ensure_access_grants(db: AsyncSession, org: Organization, admin_role: Role, member_role: Role) -> None:
    ...
    db.add_all(
        [
            Permission(code=code, group=code.split(".")[0].title(), description=code.replace(".", " ").title())
    ...
    existing_admin = set(
        (
            await db.scalars(
                select(AccessGrant.permission_id).where(
                    AccessGrant.organization_id == org.id,
                    AccessGrant.principal_type == "role",
                    AccessGrant.principal_id == admin_role.id,
                )
            )
        ).all()
    )
        ...
        (
            await db.scalars(
                select(AccessGrant.permission_id).where(
                    AccessGrant.organization_id == org.id,
                    AccessGrant.principal_type == "role",
                    AccessGrant.principal_id == member_role.id,
                )
            )
        ).all()
    )
    ...
    db.add_all(
        [
            ...
            if permissions[code].id not in existing_member
        ]
    )
...
async def seed(db: AsyncSession) -> None:
    ...
    if existing_org:
        roles = {
            role.name: role
            for role in (await db.scalars(select(Role).where(Role.organization_id == existing_org.id))).all()
        }
        if "Organization Admin" in roles and "Member" in roles:
            ...
            platform = await db.scalar(
                select(User).where(
                    User.email == get_settings().dev_admin_email.lower(), User.organization_id == existing_org.id
                )
            )
            if platform:
                ...
                membership = await db.scalar(
                    select(WorkspaceMembership.id).where(
                        WorkspaceMembership.user_id == platform.id,
                        WorkspaceMembership.organization_id == existing_org.id,
                    )
                )
                if not membership:
                    db.add(
                        WorkspaceMembership(
                            user_id=platform.id, organization_id=existing_org.id, access_level="administrator"
                        )
                    )
            await db.commit()
        return
    ...
    member_role = Role(
        ...
        permissions=[
            ...
            for p in perms
            if p.code
            in {"dashboard.view", "tasks.view", "tasks.create", "teams.view", "schedule.view", "announcements.view"}
        ],
    )
    ...
    db.add(WorkspaceMembership(user_id=admin.id, organization_id=org.id, access_level="administrator"))
    nav = [
        ("Dashboard", "Dashboard", "/dashboard", None, 0),
        ("My Work", "CheckCircle", "/tasks", "tasks.view", 10),
        ("Schedule", "CalendarMonth", "/schedule", "schedule.view", 20),
        ("Teams", "Groups", "/teams", "teams.view", 30),
        ("Announcements", "Campaign", "/announcements", "announcements.view", 40),
        ("Administration", "AdminPanelSettings", "/admin", "users.view", 90),
    ]
    db.add_all(
        [
            NavigationItem(organization_id=org.id, title=t, icon=i, route=r, required_permission=p, order=o)
    ...
    db.add_all(
        [
            Task(
                ...
                description="Confirm ownership and due dates",
                status="in_progress",
                priority="high",
            ...
            Announcement(
                ...
                body="Your shared workspace is ready.",
                published=True,
            ),
        ]
    )
